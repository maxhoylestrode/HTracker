...
router.put('/me', (req, res) => {
  const { monthly_income, split_percentage, savings_goal } = req.body || {};
  const errors = [];
  ...
  db.prepare('UPDATE users SET monthly_income = ?, split_percentage = ?, savings_goal = ? WHERE id = ?')
    .run(merged.monthly_income, merged.split_percentage, merged.savings_goal, req.session.userId);
